...
std::function<char*(size_t N)> resizeFunctional2(torch::Tensor& t) {
	// construct and return a function that resize the tensor `t` 
	// to size `N` and returns a pointer to its data
...
VoxelizeGaussiansCUDA(
	...
	const bool debug) 
// be binned to `voxelize_gaussians` in `us_gaussian_voxelization._C`
{
...
VoxelizeGaussiansBackwardCUDA(
    ...
    const float center_z,
	const bool debug)
// be binned to `voxelize_gaussians_backward` in `us_gaussian_voxelization._C`
{
